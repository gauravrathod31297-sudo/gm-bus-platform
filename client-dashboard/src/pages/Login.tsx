...
const useStyles = makeStyles({
  root: { minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: tokens.colorNeutralBackground2 },
  card: { width: '420px', padding: '40px' },
  logo: { width: '64px', height: '64px', backgroundColor: tokens.colorBrandBackground, borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px', color: 'white', fontSize: '32px' },
      ...
      <Card className={styles.card}>
        ...
        <form onSubmit={submit}>
          <Field label="Email" className={styles.field}>
            <Input value={email} onChange={(_, d) => setEmail(d.value)} contentBefore={<PersonRegular />} placeholder="you@company.com" />
          </Field>
          <Field label="Password" className={styles.field}>
            <Input type="password" value={password} onChange={(_, d) => setPassword(d.value)} contentBefore={<LockClosedRegular />} placeholder="••••••••" />
          </Field>
